<template>
    <div class="  fixed left-0  shadow-lg z-10  bg-slate-800  pt-3 pr-5 border-t rounded-r-lg">
        <ul class="p-0 font-mono">
            <li class="hover:bg-slate-700 text-white rounded-lg  py-2 px-4 mt-1">
                <span class="text-md ">
                    <i class="fa-solid fa-briefcase fa-md" style="color: white;"></i>
                    My Workspace
                </span>
            </li>
            <li class="hover:bg-slate-700 text-white p-2 mt-1 rounded-lg  py-2 px-4">
                <span class="text-md py-1">
                    <i class="fa-regular fa-bell fa-md" style="color: white;"></i>
                    Warning
                </span>
            </li>
            <li class="hover:bg-slate-700 text-white  p-2 mt-1 rounded-lg  py-2 px-4">
                <span class="text-md">
                    <i class="fa-regular fa-star fa-md" style="color: white;"></i>
                    Star
                </span>
            </li>
            <li class="hover:bg-slate-700 text-white   p-2 mt-1 rounded-lg  py-2 px-4">
                <span class="text-md">
                    <i class="fa-regular fa-message fa-md" style="color: white;"></i>
                    Messages
                </span>
            </li>
            <li class="hover:bg-slate-700 text-white  p-2 mt-1 rounded-lg  py-2 px-4">
                <span class="text-md">
                    <i class="fa-solid fa-book fa-md" style="color: white;"></i>
                    Notepad
                </span>
            </li>
            <li class="hover:bg-slate-700 text-white  p-2 mt-1 rounded-lg  py-2 px-4">
                <span class="text-md">
                    <i class="fa-solid fa-arrow-right-from-bracket"></i>
                    Log Out
                </span>
            </li>
        </ul>

    </div>
</template>

<script setup>

</script>

<style lang="scss" scoped></style>